...
fun assert(ok: OK): Boolean {
  ...
  var result = datkt.tape.assert(ctx, 0, AssertionOptions(
    op = "test",
    error = "error",
    message = "oops"
  ))
  ...
  result = datkt.tape.assert(ctx, 0, AssertionOptions(message = "oops"))
  ...
  result = datkt.tape.assert(ctx, true)
  ...
  result = datkt.tape.assert(ctx, "", AssertionOptions(
    error = Error("ERROR")
  ))
